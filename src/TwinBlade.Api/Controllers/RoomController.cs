...
public sealed class RoomController(IMediator mediator, IPlayerRepository playerRepository) : ControllerBase
{
    [HttpPost]
    ...
    {
        var playerId = await GetCurrentPlayerIdAsync(ct);
    ...
    private async Task<Guid?> GetCurrentPlayerIdAsync(CancellationToken ct)
    {
        // JWT middleware maps "username" claim to ClaimTypes.NameIdentifier due to NameClaimType = "username" config
        var cognitoId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value
                        ?? User.FindFirst("username")?.Value;

        if (string.IsNullOrWhiteSpace(cognitoId))
            return null;

        var player = await playerRepository.GetByCognitoIdAsync(cognitoId, ct);
        return player?.Id;
    }
}
